...
```{r}
totsteps <- tapply(stepsdata$steps, stepsdata$date,sum)
...
```{r}
meansteps <- tapply(stepsdata$steps,stepsdata$interval,
                                 mean,na.rm=TRUE)
```
Timeseries plot of the 5-min interval and the average number of steps taken, averaged across all days
```{r}
...
```{r}
num_na_values <- sum(is.na(stepsdata))
num_na_values 
```

Fill in missing values using the **average interval value across all days**
```{r}
na_indices <-  which(is.na(stepsdata))
imputed_values <- meansteps[as.character(stepsdata[na_indices,3])]
names(imputed_values) <- na_indices
for (i in na_indices) {
    stepsdata$steps[i] = imputed_values[as.character(i)]
}
sum(is.na(stepsdata)) 
totsteps <- tapply(stepsdata$steps, stepsdata$date,sum)
...
stepsdata$day_type <- ifelse(days == "Saturday" | days == "Sunday", 
                                "Weekend", "Weekday")
meansteps <- aggregate(stepsdata$steps,
                                    by=list(stepsdata$interval,
                                            stepsdata$day_type),mean)
